...
let materialesOriginales = []; // Para validar stock real sin consultar la API constantemente
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        // Indicador de carga visual en los selects
        Object.values(selects).forEach(s => { if(s) s.innerHTML = '<option>Cargando...</option>'; });
        ...
        if (result.success) {
            ...
            const llenar = (select, lista) => {
                ...
                lista.forEach(m => {
                    ...
                    select.innerHTML += `<option value="${m._id}" style="${color}">${m.nombre} ${avisoStock}</option>`;
                });
            };
            ...
            console.log("✅ Inventario sincronizado para cotización");
        }
    } catch (error) {
...
async function procesarCotizacion() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Lógica de Negocio: Costo Materiales x 3 + Mano de Obra
...
function mostrarResultado(data) {
    ...
    // Validación de Stock Visual Crítica
    let htmlStockAlert = "";
    ...
    data.detalles.materiales.forEach(m => {
        ...
        if (areaRequerida > stockDisponible) {
            ...
            htmlStockAlert += `<div style="background: #fee2e2; color: #b91c1c; padding: 8px; border-radius: 6px; font-size: 0.85rem; margin-top: 5px; border: 1px solid #fecaca;">
                <strong>⚠️ Agotado:</strong> ${m.nombre} (Faltan ${(areaRequerida - stockDisponible).toFixed(2)}m²)
            </div>`;
        }
    });
    ...
    document.getElementById('containerAcciones').innerHTML = `
        <div class="confirm-sale-box" style="background: #ffffff; border: 2px solid #3498db; padding: 25px; border-radius: 12px; margin-top: 25px; box-shadow: 0 4px 15px rgba(52,152,219,0.1);">
            <h4 style="margin:0 0 20px 0; color: #2980b9; font-size: 1.2rem; display: flex; align-items: center; gap: 10px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                <div class="input-group">
                    ...
                    <input type="text" id="nombreCliente" placeholder="Ej: Juan Pérez" style="width:100%; padding:12px; border-radius:8px; border:1px solid #cbd5e1; outline: none;">
                </div>
                <div class="input-group">
                    <label style="font-size: 0.8rem; font-weight: 700; color: #475569;">WHATSAPP / TEL</label>
                    <input type="text" id="telCliente" placeholder="Ej: 3001234567" style="width:100%; padding:12px; border-radius:8px; border:1px solid #cbd5e1; outline: none;">
                </div>
            </div>
...
async function facturarVenta() {
    ...
    if (!nombre) { 
        alert("⚠️ Por favor, ingresa el nombre del cliente para la factura."); 

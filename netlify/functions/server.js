     ...
     * Módulo de Servidor (Netlify Function) - Versión 13.3.6 (CONSOLIDADA)
    ...
    try {
        ...
        console.log("📦 Modelos v13.3.6 registrados exitosamente");
    } catch (err) {
    ...
    app.use((req, res, next) => {
        ...
        console.log(`📡 [v13.3.6] ${req.method} -> ${req.url}`);
        next();
    });
    ...
    try {
        ...
        router.get('/invoices', async (req, res) => {
            try {
                ...
                // --- 🔧 LIMPIEZA QUIRÚRGICA PARA LA TABLA (SIN DAÑAR LA DB) ---
                const facturasLimpias = facturas.map(f => {
                    // Si el cliente es un objeto, extraemos el nombre para que la tabla no diga [object Object]
        ...
        router.get('/health', (req, res) => {
            res.json({ status: 'OK', version: '13.3.6', db: mongoose.connection.readyState === 1 });
        });
